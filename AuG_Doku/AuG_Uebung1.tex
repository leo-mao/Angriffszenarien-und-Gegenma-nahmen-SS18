...
\begin{document}
    ...
    \section{WEP}
		...
		\item Wie funktioniert Keystream-Reuse bei WEP?
			\begin{answer}
			Bei WEP wird der Schlüssel $k$ zusammen mit einem \ref{iv} verwendet, um mit dem RC4-Algorithmus eine Byte-Schlüsselfolge zu erzeugen. Der Klartext wird mit dem Bytestrom bitweise XOR-verknüpft.\\
		...
		\begin{answer}
		Laut "Birthday-Paradox" ist die Chance, dass ein IV mit 32 bit eine Dopplung in etwa $1.2\times\sqrt{2^{32}} \approx 2^{16}$,mehr als 50\%. Obwohl dadurch die Benötigte Zeit des Angriffs verlängern wird, aber trotzdem nicht verhindern kann.   
		\end{answer}
